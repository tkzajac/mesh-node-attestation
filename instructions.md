...
1. **Istio's CSR flow** — ztunnel (Istio ambient's per-node proxy, the
   only mode this repo targets — no sidecars anywhere here) generates a
   CSR, authenticates via a Kubernetes ServiceAccount token, and gets a
   cert back from a CA. Here that CA is cert-manager's `istio-csr` agent,
...
authority; treat the CAS option as documented, not implemented.
...
- **Ambient mode only, no sidecars anywhere in this repo.** The Istio CSR
  path targets ztunnel exclusively — don't reintroduce `istio-agent`
  sidecar mode, comparisons to it, or `istioctl proxy-config`-style
  sidecar tooling into docs or manifests.